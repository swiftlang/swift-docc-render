...
// Spy window.Function to check that navigation scripts are executed.
const functionSpy = jest.spyOn(window, 'Function');
...
function setCustomScripts(customScripts) {
  ...
  fetchMock = jest.fn()
    .mockResolvedValueOnce({
      // The first call to the fetch function on each test will be to fetch custom-scripts.json:
      ok: true,
      json: jsonMock,
    }).mockResolvedValue({
      // Any subsequent calls to fetch will be to fetch the contents of each script file:
      ok: true,
      text: textMock,
    });
